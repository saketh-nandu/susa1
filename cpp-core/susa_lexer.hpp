#ifndef SUSA_LEXER_HPP
#define SUSA_LEXER_HPP
...
enum class TokenType {
    ...
    PRINT,
    LET,
    CONST_VAR,      // Renamed from CONST to avoid Windows conflict
    INT,
    STRING_TYPE,
    BOOL,
    FLOAT,
    ...
    STATIC,         // NEW: static variables
    ENUM,           // NEW: enum support
...
private:
    ...
    std::unordered_set<std::string> keywords = {
        "print", "let", "if", "else", "elif", "while", "for", "in",
        "func", "return", "yield", "async", "await", "class", "start", "end", "loop", "times",
        "break", "continue", "try", "catch", "with", "install", "from", "add",
        "share", "as", "use", "python", "assert",
        "true", "false", "null", "and", "or", "not",
        "int", "string", "bool", "float", "double", "char",
        "const", "do", "switch", "case", "default", "lambda", "static", "enum"
    };
    ...
    Token make_template_string() {
        ...
        // Mark this as a template string by prefixing with special marker
        str = "\x01TEMPLATE\x01"; // Special marker for template strings
    ...
    TokenType get_keyword_type(const std::string& keyword) {
        ...
        if (keyword == "const") return TokenType::CONST_VAR;
        ...
        while (current_char != '\0') {
            ...
            switch (current_char) {
                ...
                case '>':
                    ...
                    break;
                case '?':
                    tokens.push_back(Token(TokenType::IDENTIFIER, "?", line, start_col));
                    advance();
                    break;
                case '(': tokens.push_back(Token(TokenType::LPAREN, "(", line, start_col)); advance(); break;
                case ')': tokens.push_back(Token(TokenType::RPAREN, ")", line, start_col)); advance(); break;
                case '[': tokens.push_back(Token(TokenType::LBRACKET, "[", line, start_col)); advance(); break;
                case ']': tokens.push_back(Token(TokenType::RBRACKET, "]", line, start_col)); advance(); break;
                case '{': tokens.push_back(Token(TokenType::LBRACE, "{", line, start_col)); advance(); break;
                case '}': tokens.push_back(Token(TokenType::RBRACE, "}", line, start_col)); advance(); break;
                case ',': tokens.push_back(Token(TokenType::COMMA, ",", line, start_col)); advance(); break;
                case ':': tokens.push_back(Token(TokenType::COLON, ":", line, start_col)); advance(); break;
                case '.': 
                    if (peek() == '.' && peek(1) == '.') {
                    ...
                    break;
                case ';': tokens.push_back(Token(TokenType::SEMICOLON, ";", line, start_col)); advance(); break;
                case '&':
