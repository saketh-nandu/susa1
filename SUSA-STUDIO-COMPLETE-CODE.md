# SUSA Studio - Complete Code

## File 1: `src/components/SUSAStudio.tsx`

Copy this ENTIRE code into `SUSAStudio.tsx`:
      ...
      episodes: [
        {
          ...
          title: "Introduction",
          duration: "3:45",
          videoUrl: "https://www.dropbox.com/scl/fi/your-video-id/episode1.mp4?rlkey=your-key&dl=1",
          available: true
        },
...
## File 2: Update `src/components/SUSAHero3D.tsx`

### Step 1: Change the import at the top
Find this line:
```typescript
import LaunchTheater from "./LaunchTheater";
```
...
Find:
```typescript
const [showLaunchTheater, setShowLaunchTheater] = useState(false);
```
...
### Step 3: Update the trigger handler
Find:
```typescript
const handleCinematicTrigger = () => {
  setCinematicTransition(true);
  setTimeout(() => {
    setShowLaunchTheater(true);
...
<AnimatePresence>
  {showLaunchTheater && (
    <LaunchTheater
      ...
      onClose={() => setShowLaunchTheater(false)}
    />
  )}
</AnimatePresence>
```
...
### Step 5: Update "introducing" text to vanish together
Find the "introducing" section and update it to:
```typescript
<motion.div
...
1. Copy the SUSAStudio code above into `SUSAStudio.tsx`
2. Make the 5 changes in `SUSAHero3D.tsx`
3. Update the video URL in line 48 of SUSAStudio.tsx
4. Build and deploy:
```bash
